@if($template->user_id == Auth::user()->id || request()->is('admin/*'))
    ...
    <div class="dropdown-menu {{ (__('lang_dir') == 'rtl' ? 'dropdown-menu' : 'dropdown-menu-right') }} border-0 shadow">
        ...
        @if(request()->is('admin/*'))
            <a class="dropdown-item d-flex align-items-center" href="{{ request()->is('admin/*') ? route('admin.documents', ['template_id' => $template->id]) : route('documents', ['template_id' => $template->id]) }}">@include('icons.article', ['class' => 'text-muted fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Documents') }}</a>
        @endif
